// Maze.h - XMaze 绫诲０鏄?// 杩樺師鑷?GameServer.exe IDA 鍙嶇紪璇?
#pragma once
...
private:
    ...
    float m_fWarpTime;
    int m_nSendTimeSec;
    void* m_pCurInfo;  // NOTE: STMagePotalBox* or similar
    class XMaze* m_pMaze;
    int m_nJumpID;
};
...
protected:
    ...
    // Cutscene Manager - 浣跨敤鎸囬拡閬垮厤涓嶅畬鏁寸被鍨嬮棶棰?    // IDA: m_cutSceneManager at fixed offset in XMaze
    ...
    void* m_pNavMeshResource;  // NOTE: HavokNavMeshResource*

    // Maze Resource
    void* m_pMazeResource;  // NOTE: VBaseResourceLump*

    // System Actor (for scene attacks)
    CMonster* m_pSystemActor;

    // Script Instance
    void* m_pScriptInstance;  // NOTE: IVScriptInstance*
    ...
    void* m_pHiddenEvent;  // NOTE: proper type for hidden event
    ...
    std::map<int, void*> m_mapGateBox;  // NOTE: STMageGateBox*
    std::map<int, void*> m_mapLuaFunctionBox;  // NOTE: STLuaFunctionBox*
    ...
    std::list<void*> m_lstSilhouetteObject;  // NOTE: hkaiPointCloudSilhouetteGenerator*
    ...
    std::list<void*> m_listWaitForRecvInfo;  // NOTE: proper type

    // Logic Timers (IDA confirmed)
    std::list<void*> m_arLogicTimers;      // NOTE: LogicTimer* proper type
    std::list<void*> m_arWaitLogicTimers;  // NOTE: LogicTimer* proper type

    // Vectors
    std::vector<int> m_vecActiveLastSectorID;
    std::list<CMonster*> m_lstChangeMonster;
    std::list<int> m_listDieMonsterSpawnBoxID;
    std::list<void*> m_listSyncSpawnActive;  // NOTE: proper type
    std::vector<int> m_vecLuaValue;
    std::vector<std::string> m_vecLuaValues;
    std::vector<CGameWorldMode*> m_vecGameRules;
    std::vector<void*> m_vecRoguelikeRandomShopStat;  // NOTE: proper type
    ...
    std::map<std::uint32_t, std::vector<void*>> m_mapDropList;  // NOTE: proper type
    std::map<std::uint32_t, bool> m_mapCutsceneOff;

    // Roguelike
    std::map<int, CSector*> m_mapRogueSector;
    std::map<int, void*> m_mapRoguelikeRandomPocketInfo;  // NOTE: proper type
    std::map<int, std::vector<int>> m_mapRoguelikePortalBuffList;
    std::map<int, int> m_mapRoguelikePortalBuff;
    std::map<int, void*> m_mapRandomBuff;  // NOTE: proper type
    std::map<int, void*> m_mapTimeStepTimer;  // NOTE: proper type

    // Game Trap
    std::map<std::uint32_t, void*> m_mapGameTrapObject;  // NOTE: proper type
    std::map<int, void*> m_mapGameTrapObjectGroup;  // NOTE: proper type
    ...
    // Script Die Monster
    std::map<std::uint32_t, void*> m_mapCallScriptDieMonster;  // NOTE: proper type
    ...
    // Cell Position Manager
    // NOTE: CCellPosMgr m_CellPosMgr;  // Need proper type definition
