...
// TODO: 推测结果：默认服务名/描述尚未在 PDB 中直接核实，仅按模块语义回填。
char g_serviceName[256] = "LoginServer";
char g_description[256] = "SoulWorker LoginServer";
#ifdef _WIN32
...
#else
    const std::string message = std::system_category().message(static_cast<int>(lastError));
    const std::size_t length = message.size();
    char* buffer = static_cast<char*>(std::malloc(length + 1));
    if (!buffer) {
        return 0;
    }
    std::memcpy(buffer, message.c_str(), length + 1);
    *msgBuf = buffer;
    return length;
#endif
}
...
bool InstallService(const char* binaryPathName, const char* serviceName, std::uint32_t startType) {
    ...
    LogHelper::LogError("game.system",
                        "InstallService is only supported on Windows service environments");
    return false;
#endif
}
...
bool UninstallService(const char* serviceName) {
    ...
    (void)serviceName;
    LogHelper::LogError("game.system",
                        "UninstallService is only supported on Windows service environments");
    return false;
#endif
}
...
std::uint64_t SERVICE_HANDLER(unsigned int fdwControl,
    ...
    switch (fdwControl) {
        case 1:
            ...
            TXSingleton<XLoginServer>::Instance()->Shutdown(0xFFFFFFFFu);
            break;
        case 2:
