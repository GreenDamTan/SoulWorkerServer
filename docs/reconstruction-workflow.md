...
##### 2.5.1）函数重载检查规则（极高优先级）

**问题背景**：C++ 允许同名函数有多个重载版本（不同参数列表），IDA 中可能同时存在多个同名但地址不同的函数。

**必须遵守以下规则**：

1. **遇到函数签名不匹配时**，首先怀疑是否存在函数重载：
   * 使用 IDA list_functions 搜索同名函数
   * 检查是否有多个地址对应同一函数名
   * 对比每个重载版本的参数列表

2. **在 func-index 中记录重载**：
   * 每个重载版本必须单独占一行
   * 在 
ote 列标注参数差异，例如：`(3-param)`、`(4-param)`
   * 示例：
     | CGocInventory::UpgradeSocket | 0x1400D2720 | ... | (3-param) |
     | CGocInventory::UpgradeSocket | 0x1400D29F0 | ... | (4-param) |

3. **反编译时必须核实地址**：
   * 不允许假设"同名函数只有一个版本"
   * 不允许混淆不同重载版本的实现
   * 不允许用错误重载版本的代码覆盖另一个版本

4. **常见重载场景**：
   * 同名函数不同参数数量（如 3-param vs 4-param）
   * const 成员函数 vs 非 const 成员函数

**错误示例**：
* func-index 只记录一个条目，遗漏其他重载版本
* 用 3-param 版本的实现去验证 4-param 版本的条目
